...
- [Development guidelines](#development-guidelines)
    - [General ILSCoin](#general-bitcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
ILSCoin uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
ILSCoin is a multi-threaded application, and deadlocks or other
...
ILSCoin can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as ILSCoin, where everyone uses
...
pay attention to for reviewers of ILSCoin code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: ILSCoin uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of ILSCoin, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against ILSCoin so that they can be integrated
...
file descriptors, it will cause problems with ILSCoin's `select()` loop, because
...
compatibility between nodes. This happened in ILSCoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to ILSCoin's
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of ILSCoin, the wallet was always
